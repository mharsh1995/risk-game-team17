...
public class Aggressive extends PlayerBehavior {
	 ...
	 */
	public Aggressive(GamePhaseController gamePhaseController) {
	...
	private void attack(Country attacking, Country defending, Player player) {
		...
		if (TournamentModel.isTournamentMode) {
			RiskDiceController diceController = new RiskDiceController(dice, this);
			diceController.automateDiceRoll();
		} else {
			RiskDiceController diceController = new RiskDiceController(dice, this, this.gamePhaseController);
			diceController.automateDiceRoll();
		}
	}
